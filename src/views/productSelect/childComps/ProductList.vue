<template>
  <div class="product-list">
    ...
    <!-- 分页视图-->
    <prodct-pagination @onsend="getPagination"/>
    <!-- 添加产品对话框 -->
    <el-dialog title="添加产品" :visible.sync="dialogAddProductVisible" width="80%" :before-close="handleClose">
      <!--商品类目选择对话框 -->
      <el-dialog width="60%" title="商品类目选择" :visible.sync="dialogProductCategoryVisible" append-to-body>
        <!-- 类目tree -->
        <el-tree :load="loadTreeNodes" :props="defaultProps" lazy highlight-current @node-click="handleNodeClick"></el-tree>
      ...
      <el-form ref="form" :model="product" label-width="80px">
        <el-form-item label="商品类目">
          <el-button style="float:left" type="primary" @click="productCategoryHandler">选择类目</el-button>
        ...
        <el-form-item label="商品图片">
          ...
          <!-- 上传图片的对话框 -->
          <el-dialog title="上传图片" :visible.sync="dialogUploadImgVisible" width="40%" :before-close="handleClose" append-to-body>
            <product-upload @onUpload="getUploadHandler"/>
            <span slot="footer" class="dialog-footer">
        ...
        <el-form-item label="商品描述">
          <product-ueditor :productDesc="productDesc" @onProductDesc="getProductDesc"/>
        </el-form-item>
      </el-form>
      <!-- 规格参数添加视图 -->
    ...
    methods: {
      ...
      handleDelete(index, row) {
        this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
          ...
          this.$api.getDeleteItemById({ itemId: row.id }).then(res => {
            if (res.data.status === 200) {
              this.$message({
                type: "success",
                message: "删除成功!"
      ...
      addParamsHandler(){
        ...
        // 获取规格参数数据
        this.$api.getSelectItemParamByItemCatId({id:this.paramsObj.id}).then(res => {
          if(res.data.status === 200){
      ...
      loadTreeNodes(node, resolve) {
        ...
        if (node.level >= 1) {
          // 点击之后触发
          this.$api.getSelectItemCategoryByParentId({ id: node.data.id }).then(res => {
            if (res.status === 200) {
              return resolve(res.data.data);
            }
          }).catch(error => {
