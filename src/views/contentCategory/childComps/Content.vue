<template>
  <div>
    <el-button class="addBtn" @click="addDialogVisible = true" type="primary"
               v-if="addBtnShow">添加{{ addContent }}</el-button>
    ...
    <!-- 添加弹出框 -->
    <el-dialog title="添加内容" :visible.sync="addDialogVisible" width="60%" :before-close="handleClose">
      <el-form ref="form" :model="form" label-width="80px">
        ...
        <el-form-item label="图片上传">
          ...
          <el-dialog append-to-body title="上传图片" :visible.sync="uploadDialogVisible" width="30%" :before-close="handleClose">
            <el-upload ref="upload" class="upload-demo" action="/product_api/file/upload" :limit="1"
                       :auto-upload="false" :on-success="uploadHandler">
              <el-button slot="trigger" size="small" type="primary" round>添加图片</el-button>
              <el-button style="margin-left: 10px;" size="small" type="success" round @click="submitUpload">
                上传
  ...
  export default {
    ...
    watch: {
      currentId(value, oldValue) {
        ...
        this.$api.getSelectTbContentAllByCategoryId({ categoryId: value.id }).then(res => {
