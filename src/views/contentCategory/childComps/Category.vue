<template>
  <div>
    <el-tree :props="defaultProps" :load="loadTreeNodes" lazy highlight-current
             :render-content="renderContent" :expand-on-click-node="false" @node-click="handleNodeClick"></el-tree>
    <!-- 添加 -->
    <el-dialog title="添加名称" :visible.sync="categoryVisible" width="30%" :before-close="handleClose">
      <span>
    ...
    <!-- 修改 -->
    <el-dialog title="修改名称" :visible.sync="updateCategoryVisible" width="30%" :before-close="handleClose">
      <span>
  ...
  export default {
    ...
    methods: {
      loadTreeNodes(node, resolve) {
        ...
        if (node.level >= 1) {
          // 点击之后触发
          this.$api.getSelectContentCategoryByParentId({ id: node.data.id }).then(res => {
            if (res.status === 200) {
              return resolve(res.data.data);
            }
          }).catch(error => {
